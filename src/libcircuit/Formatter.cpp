...
namespace circuit
{
	Formatter::Formatter(Graph const &g, std::ostream *out_stream)
		: m_out_stream(out_stream), m_current_raw(0), m_current_column(0)
	{
	...
	{
		while (m_current_column != 0) this->propagate_value('U');
	}


	void Formatter::propagate_value(bool value) { this->propagate_value(value ? '1' : '0'); }

	Tester::Tester(Graph const &g, std::ostream *out_stream) : Formatter(g, out_stream) {}
	...
	void Tester::propagate_value(char value)
	{
		...
		else { ++this->m_current_column; }
	}

	TablePrinter::TablePrinter(Graph const &g, std::ostream *out_stream)
		: Formatter(g, out_stream), m_separator("|"), m_table_width(1)
	{
		m_table_width = std::accumulate(this->m_inputs.begin(),
			this->m_inputs.end(),
			m_table_width,
			[](size_t value, std::string const &e) { return value + e.size() + 3; });
		m_table_width = std::accumulate(this->m_outputs.begin(),
			this->m_outputs.end(),
			m_table_width,
			[](size_t value, std::string const &e) { return value + e.size() + 3; });
		this->print_header();
	...
	void TablePrinter::propagate_value(char value)
	{
		...
		*this->m_out_stream << std::left;
		auto const &current_column = this->m_current_column < this->m_inputs.size()
						     ? this->m_inputs[this->m_current_column]
						     : this->m_outputs[this->m_current_column - this->m_inputs.size()];
	...
	void TablePrinter::print_header() const
	{
		...
		*this->m_out_stream << this->m_separator;
		...
		{
			*this->m_out_stream << std::left;
			*this->m_out_stream << std::setw(static_cast<std::streamsize>(m_input.size() + 2)) << m_input;
			*this->m_out_stream << this->m_separator;
		}
		...
		{
			*this->m_out_stream << std::left;
			*this->m_out_stream << std::setw(static_cast<std::streamsize>(m_output.size() + 2)) << m_output;
			*this->m_out_stream << this->m_separator;
		}
		*this->m_out_stream << '\n';
		this->print_horizontal_line();
